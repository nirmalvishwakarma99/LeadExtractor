...
#         # 1. Guarantee Business Status is preserved
#         if 'Business Status' not in self.target_columns:
#             if 'Business/Company Address' in self.target_columns:
#                 idx = self.target_columns.index('Business/Company Address')
#                 self.target_columns.insert(idx + 1, 'Business Status')
#             else:
#                 self.target_columns.append('Business Status')

#         # 2. Guarantee Mobile slots exist
#         for col_name in ['Mobile number 2', 'Mobile number 3', 'Extra Mobile Number']:
#             if col_name not in self.target_columns:
#                 ref_col = 'Mobile number 3' if col_name == 'Extra Mobile Number' else 'Mobile number'
#                 if ref_col in self.target_columns:
#                     idx = self.target_columns.index(ref_col)
#                     self.target_columns.insert(idx + 1, col_name)

#         # 3. Guarantee Email slots exist
#         for offset, col_name in enumerate(['Email 2', 'Email 3', 'Extra Email'], start=1):
#             if col_name not in self.target_columns and 'Email' in self.target_columns:
#                 idx = self.target_columns.index('Email')
#                 self.target_columns.insert(idx + offset, col_name)
...
#     #     # Guarantee mobile 2 & 3 exist
#     #     if 'Mobile number 2' not in self.target_columns:
#     #         if 'Mobile number' in self.target_columns:
#     #             idx = self.target_columns.index('Mobile number')
#     #             self.target_columns.insert(idx + 1, 'Mobile number 2')
#     #             self.target_columns.insert(idx + 2, 'Mobile number 3')

#     #     # Add website enrichment columns so they are never stripped
#     #     extra_cols = [
#     #         'Location Link', 'Owner Name (scraped)', 'Address (scraped)',
#     #         'Pincodes (scraped)', 'RERA No (scraped)'
#     #     ]
#     #     for c in extra_cols:
#     #         if c not in self.target_columns:
#     #             self.target_columns.append(c)
...
#             # --- MOBILE NUMBER SLOTS (1, 2, 3 + EXTRA SLUG) ---
...
#             # --- EMAIL SLOTS (1, 2, 3 + EXTRA OVERFLOW) ---
#             'Email': record.email,
